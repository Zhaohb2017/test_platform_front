          ...
          <el-form-item label="创房选项" width="290" min-width="180" header-align="center" prop="c_option">
                <el-card class="box-card">
                    ...
                    <div v-show="ShowFlag">
                        <template>
                            <span>抽牌：</span>
                            <el-radio v-model="editCaseForm.c_option.o_chouforty" :label=20>抽牌20张</el-radio>
                            <el-radio v-model="editCaseForm.c_option.o_chouforty" :label=30>抽牌30张</el-radio>
                            <el-radio v-model="editCaseForm.c_option.o_chouforty" :label=40>抽牌40张</el-radio>
                    ...
                    <div>
                        <template>
                            <span>可选项：</span>
                            <el-checkbox v-model="editCaseForm.c_option.o_wujianghu"         :label=1>无将胡</el-checkbox>
                            <el-checkbox v-model="editCaseForm.c_option.o_queyise"           :label=1>缺一色</el-checkbox>
                            <el-checkbox v-model="editCaseForm.c_option.o_pengpenghu"        :label=1>碰碰胡</el-checkbox>
                            <el-checkbox v-model="editCaseForm.c_option.o_qingyise"          :label=1>清一色</el-checkbox>
                            <el-checkbox v-model="editCaseForm.c_option.o_jiangjianghu"      :label=1>将将胡</el-checkbox>
                            <el-checkbox v-model="editCaseForm.c_option.o_qixiaodui"         :label=1>七小对</el-checkbox>
                            <el-checkbox v-model="editCaseForm.c_option.o_dadiaoche"         :label=1>大吊车</el-checkbox>
                        </template>
                    </div>
                    <div>
                        <template>
                            <span>请选择玩法：</span>
                            <el-checkbox v-model="editCaseForm.c_option.o_youpaibihu" :label=1 @change="youpaibihu">有牌必胡</el-checkbox>
                            <el-checkbox v-model="editCaseForm.c_option.o_denghu" :label=2 @change="denghu">等胡</el-checkbox>
                            <el-checkbox v-model="editCaseForm.c_option.o_huDiejia" :label=2 >大小胡不叠加</el-checkbox>
                        </template>
                    </div>
                    <div>
                        <template>
                            <span>请选择封顶：</span>
                            <el-radio v-model="editCaseForm.c_option.o_fengding" :label=0>不封顶</el-radio>
                            <el-radio v-model="editCaseForm.c_option.o_fengding" :label=3>3番</el-radio>
                            <el-radio v-model="editCaseForm.c_option.o_fengding" :label=4>4番</el-radio>
                            <el-radio v-model="editCaseForm.c_option.o_fengding" :label=5>5番</el-radio>
                        </template>
                    </div>
                    <div>
                        <template >
                            <span>请选择玩法：</span>
                            <el-radio-group v-model="editCaseForm.c_option.o_zhuaniao" @change="ZhuaNiao">
                                <el-radio  label='不抓鸟'>不抓鸟</el-radio>
                                <el-radio  label='159中鸟'>159中鸟</el-radio>
                                <el-radio  label='庄家159中鸟'>庄家159中鸟</el-radio>
                            </el-radio-group>
                        </template>
                    </div>


                    <div v-show="ShowZhuaNiao">
                        <span>玩法：</span>
                        <el-radio-group v-model="editCaseForm.c_option.o_zhongNiaoJiaFen">
                            <el-radio label='中鸟相加' @change="ZhongNiaojiafen">中鸟相加</el-radio>
                            <el-radio label='中鸟翻倍' @change="ZhongNiaofanbei">中鸟翻倍 </el-radio>
                        </el-radio-group>
                    </div>
                    <div v-show="ShowZhongNiaoScore">
                        <span>请选择玩法：</span>
                        <el-radio-group v-model="editCaseForm.c_option.o_hujiafen">
                            <el-radio label='加胡牌分'>加胡牌分</el-radio>
                            <el-radio :label=1>加1分</el-radio>
                            <el-radio :label=2>加2分</el-radio>
                            <el-radio :label=3>加3分</el-radio>
                            <el-radio :label=4>加4分</el-radio>
                        </el-radio-group>
                    </div>
                    <div v-show="ShowZhongNiaoScore">
                        <span>抓鸟：</span>
                        <el-radio-group v-model="editCaseForm.c_option.o_Zhuaniao">
                            <el-radio :label=1>抓1鸟</el-radio>
                    ...
                    <div v-show="ShowZhongNiaoDouble">
                        <span>抓鸟：</span>
                        <el-radio-group v-model="editCaseForm.c_option.o_Zhuaniao">
                            <el-radio :label=1>抓1鸟</el-radio>
                            <el-radio :label=2>抓2鸟</el-radio>
  ...
  export default {
    ...
    data() {
      return {
        ...
        re_data: '',
          ShowZhuaNiao:false,
          //抓鸟显示
          ShowZhongNiaoScore:false,
          ShowZhongNiaoDouble: false,
          // 创房选项二人时显示抽牌标记位
          show_club_id:false,
          ShowFlag: false,
          //加倍选项标记
    ...
    mounted() {
        if(this.current_data){
            ...
            console.log("c_purpose", dataObj ,typeof dataObj);
            console.log("FFFFF", this.editCaseForm.c_operate);
    ...
    methods: {
      Closeed(){
        ...
        youpaibihu(val){
            if(val==true){
                this.editCaseForm.c_option.o_denghu = false;

            }
        },
        denghu(val){
            if(val==true){
                this.editCaseForm.c_option.o_youpaibihu = false;
            }
        },
        // 添加新的翻倍积分选项
        doublePlusNew(score){
        ...
        ZhuaNiao(val){
            if(val !='不抓鸟'){
                ...
                this.ShowZhongNiaoScore = false;
                this.ShowZhongNiaoDouble = false;
                this.editCaseForm.c_option.o_hujiafen = 0;
                this.editCaseForm.c_option.o_Zhuaniao = 0;
                this.ShowZhuaNiao = false;
                this.editCaseForm.c_option.o_zhongNiaoJiaFen = '';
            }

        },
        ZhongNiaojiafen(val){
            if(val == '中鸟相加'){
                this.ShowZhongNiaoScore = true;
                this.editCaseForm.c_option.o_hujiafen = 1;
                this.editCaseForm.c_option.o_Zhuaniao = 1;
                this.ShowZhongNiaoDouble = false;
            }
        },
        // 加倍选项判断
