<template>
    <div class="bugs">
        ...
        <el-table
            ...
            <el-table-column label="玩家ID" width="100" prop="c_user" header-align="center" align="center">
                <template slot-scope="scope">
...
<script>
    ...
    import AddBugs from './add_hzmj'
    import EditBugs from './edit_hongzhongmajiang'
        ...
        methods: {
            ...
            getBugsList(){
                ...
                axios({
                    ...
                    for (var i in that.caseData){
                        ...
                        for(var num in that.caseData[i]["c_operate"]){
                            for(var card in that.caseData[i]["c_operate"][num]['card']){
                                that.caseData[i]["c_operate"][num]['card'][card] = that.caseData[i]["c_operate"][num]['card'][card].slice(0,-1);
                            }
                        }
            ...
            ChangeEditFlag(row){
                if(this.$store.state.user != null){
                    ...
                    console.log("FFFF",new_data);
                    console.log("FFFF",new_data.c_option ,typeof new_data.c_option);
