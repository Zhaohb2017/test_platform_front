<template>
  <div class="bugs">
    ...
    <div class="head">
      ...
      <el-input type="text" prefix-icon="el-icon-search" required style="width:200px;" v-model="search_data"
    ...
    <el-table
      ...
      <el-table-column header-align="center" label="操作" align="center">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="text" :disabled="scope.row.isRunning" @click="RunCase(scope.$index, scope.row)">
            运行
          </el-button>
    ...
    <EditBugs :visible.sync="edit_show_flag" v-if="edit_show_flag" :current_data="current_data"
              @reload="reload"></EditBugs>

    <Reports :visible.sync="report_show_flag" v-if="report_show_flag" :report_current_data="report_current_data"
             @reload="reload"></Reports>
...
<script>
  ...
  import AddBugs from './add_yuganmajiang'
  import EditBugs from './edit_yuganmajiang'
    ...
    methods: {
      ...
      ChangeEditFlag(row) {
        if (this.$store.state.user != null) {
          ...
          new_data.c_options = eval('(' + row.c_option
            .replace("玩法1", "o_wanfa1")
            .replace("局数", "o_round")
            .replace("人数", "o_player")
            .replace("玩法2", "o_wanfa2")
            .replace("封顶", "o_fengding")
            .replace("荒庄", "o_huangzhuang")
            .replace("首局坐庄", "o_shoujuzuozhuang")
            .replace("宝分", "o_baofen")
            .replace("玩法3", "o_wanfa3")
            .replace("防作弊", "o_fangzuobi") + ')')
      ...
      RunCase(index, row) {
        if (this.$store.state.user != null) {
          console.log("id： ", row.isRunning)
          let c_id = row['c_id']
          let that = this;
          row.isRunning = true;
          // that.isRunning = row.c_id
          that.$message.success("用例正在运行.")
          console.log("isRunning: %s", that.isRunning)
          axios({
            method: 'get',
            url: '/api/cases/run',
            params: {
